...
Главный расход — история стакана: **~7.5 КБ на снимок**, 10 снимков за цикл,
то есть примерно **70 МБ в сутки** при непрерывной работе.

Обрезка включена по умолчанию: `ORDERBOOK_RETENTION_DAYS=3` в `.env`, чистка
раз в час вместе с обновлением watchlist, с `VACUUM` — место возвращается
файловой системе. При трёх днях база стабилизируется около **200 МБ**.

На диске 10 ГБ можно смело ставить 7–14 дней. Если диска мало, `1` тоже
рабочий вариант: сигналам нужны минуты истории, глубина нужна только для
разбора сделок постфактум.
