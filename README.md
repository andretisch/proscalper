...
| Переменная | По умолчанию | Назначение |
|------------|--------------|------------|
| `ORDERBOOK_RETENTION_DAYS` | `3` | Хранение истории стакана; `0` отключает обрезку |

Снимок стакана весит ~7.5 КБ, за цикл их 10 — примерно **70 МБ в сутки** непрерывной работы. Обрезка идёт раз в час с `VACUUM`, при трёх днях база стабилизируется около 200 МБ. Логи ограничены ротацией: 10 МБ × 6 файлов.
