...
**Цикл (~90 с):** торговля и сигналы. Стакан в SQLite пишется **отдельно каждые ~15 с** (фон), чтобы `recent_range_pct` и разбор сделок опирались на плотную историю, а не на один снимок за цикл.
