...
class OllamaClient:
    ...
    def decide_trade(self, signal: dict[str, Any], context: str) -> dict[str, Any]:
        system = (
            ...
            "S2 против сильного тренда — no_go. Свежая/тонкая плотность — no_go. "
    ...
    def watchlist_pick(
        ...
        system = (
            ...
            "Критерий — размах цены, который окупает taker round-trip 0.11%: "
            "нужны движение и оборот одновременно. "
            "BTC/ETH обычно НЕ подходят: стакан слишком плотный, "
            "внутридневной шум меньше комиссии. Не добавляй их без причины."
        )
        raw = self.chat(
            [
                {"role": "system", "content": system},
